...
# ROOT_DIR = "./data/v1"
# TARGET_DIR = "./data/my_train_dataset"
# FOLD_FILE_SAVE = "./data"
# LABEL_NAMES = ["a", "b", 'c']
...
class MobileNetZhou(nn.Module):
    def __init__(self, num_classes, pool_type="avg", multi_sample=False, export=False):
        ...
        #     nn.Dropout
        #     nn.Linear(out_shape, out_shape//2, bias=False),
        #     nn.BatchNorm1d(out_shape//2),
        #     nn.ReLU(inplace=True),
        # )
        # self.classifier20 = nn.Sequential(
        ...
        # output_channel = 512
        # self.classifier20 = nn.Sequential(
        #     nn.Linear(out_shape, output_channel),
        #     # Swish_module(),
        #     nn.ReLU(inplace=True)
        # )
    ...
    def forward(self, x):
        ...
        # output = torch.mean(
        #     torch.stack(
        #         [self.classifier2(x) for _ in range(5)],
        #         dim=0,
        #     ),
        #     dim=0,
        # )
...
def make_kfold(n_split=5):
    ...
    for fold, (train_idxs, val_idxs) in enumerate(kf.split(X, labels)):
        ...
        mmcv.dump(train_fns, f"{FOLD_FILE_SAVE}/train_{fold}.pkl")
        mmcv.dump(val_fns, f"{FOLD_FILE_SAVE}/val_{fold}.pkl")
...
def prepare_data(source_dir, target_dir, clean=True):
    ...
    def dump_images(org_height, org_width, new_height, new_width, heights, widths, ratio):
        iid = 0
        # tt = Compose([AspectResize(int(HEIGHT * (256 / 224)), int(WIDTH * (256 / 224)), p=1),])
...
class JISHIDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        #     g_mask = self.grid_mask_gen(image=image)['image']
        #     g_mask_torch = torch.from_numpy(g_mask).contiguous().float()
        #     return image_torch, label_torch, g_mask_torch
        if self.test_mode:
            return image_torch, label_torch, image_name
...
class CustomLossfn(nn.Module):
    def __init__(self):
        super().__init__()
        self.crit1 = DenseCrossEntropy()
        self.crit2 = FocalCrossEntropy()

    def forward(self, x, target, ohem=None):
        loss1 = self.crit1(x, target, ohem)
        loss2 = self.crit2(x, target)
...
def train_one(dataloader, model, device, optimizer, scheduler=None, mixup=False, cutmix=False, ema_model=None, need_apex=True, ohem=None):
    ...
    for bi, data in enumerate(dataloader):
        ...
        loss = config.loss_fn(outputs, labels_onehot, ohem)
...
def val_one(dataloader, model, device):
    ...
    with torch.no_grad():
        for bi, data in enumerate(dataloader):
            ...
            labels_onehot = labels_onehot.to(device)
            outputs = model(images)
            loss = config.loss_fn(outputs, labels_onehot)
...
def run(train_fns, val_fns, fold):
    ...
    for epoch in range(config.epochs):
        train_one(train_loader, model, device, optimizer, scheduler, mixup=config.MIXUP, cutmix=config.CUTMIX, ohem=None)
        scores, losses = val_one(val_loader, model, device)
        es(scores, model, f"/project/train/models/v1p/model_{fold}.pth")
        # if es.early_stop:
        #     print("Early Stopping...........")
        #     break

    device = "cuda"
    model.load_state_dict(torch.load(f"/project/train/models/v1p/model_{fold}.pth"))
    model.eval()
    with torch.no_grad():
        for bi, data in enumerate(val_loader):
            images, labels, image_names = data[0], data[1].squeeze(), data[2]
...
class Config():
    def __init__(self):
        ...
        # self.epochs = 5 + 5 ## 5epochs for EMA
        ...
        self.transforms = Compose([
            ...
            # AutoAug(p=1),
            ShiftScaleRotate(shift_limit=0.15, scale_limit=0.15, rotate_limit=0, p=0.5),
...
if __name__ == '__main__':
    ...
    make_kfold(n_split=5)

    results = {}



    os.makedirs("/project/train/models/v1p/", exist_ok=True)
    os.makedirs("/project/train/models/v1_ema/", exist_ok=True)

    if config.run:
        for fold in range(5):
            train_fns, val_fns = mmcv.load(f"{FOLD_FILE_SAVE}/train_{fold}.pkl"), mmcv.load(f"{FOLD_FILE_SAVE}/val_{fold}.pkl")
            run(train_fns, val_fns, fold)


    torch.save(results, "/project/train/models/v1_ema/oof.pth")
