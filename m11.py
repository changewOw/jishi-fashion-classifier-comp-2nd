...
class JISHIDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        if self.test_mode:
            ...
            label_np = np.zeros((config.num_classes), dtype=np.float32)
            label_np[label] += 0.8
            label_np[oof_p[image_name]] += 0.2
            label_torch = torch.from_numpy(label_np).float()
...
def train_one(dataloader, model, device, optimizer, scheduler=None, mixup=False, cutmix=False, ema_model=None, need_apex=True, ohem=None):
    ...
    for bi, data in enumerate(dataloader):
        images, labels_onehot = data[0], data[1].squeeze()
...
def run(train_fns, val_fns):
    ...
    for epoch in range(n_splits):
        train_one(train_loader, model, device, optimizer, None, mixup=False, cutmix=False, ohem=None)
    ...
    for sss in [torch.load(f"/project/train/models/v1_ema/ema_model_{config.fold}_{epoch}.pth") for epoch in range(1,n_splits)]:
...
if __name__ == '__main__':
    ...
    oof_p = torch.load("/project/train/pre-trained-models/v1_ema/oof.pth")
